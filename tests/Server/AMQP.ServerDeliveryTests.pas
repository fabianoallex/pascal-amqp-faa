...
// Espera a escritora esvaziar e devolve os frames escritos ate agora.
...
begin
  ...
  try
    LMem := TBytesStream.Create(AStream.Bytes);
    try
...
begin
  ...
  try
    ...
    try
      ...
      Assert.AreEqual(2, LAlvo.NoteResolved(2, True), 'multiple ate a tag 2');
      Assert.AreEqual(2, LAlvo.UnackedCount, 'sobraram duas');
      Assert.AreEqual(0, LAlvo.NoteResolved(2, False),
        'tag ja resolvida nao conta de novo');
      Assert.AreEqual(2, LAlvo.NoteResolved(0, True),
