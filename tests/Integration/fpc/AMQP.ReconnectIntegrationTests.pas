...
type
  TAMQPReconnectIntegrationTests = class(TTestCase)
  private
...
begin
  ...
  LParams.ConnectionName := 'pascal-amqp-faa-recon-test-fpc';
...
begin
  ...
  AssertTrue('deveria consumir antes da queda', ReceivedContains('antes-da-queda'));
...
begin
  ...
  try
    ...
    AssertFalse('WaitForConfirms deve reportar False para publishes perdidos na queda',
      LChan.WaitForConfirms(3000));
  finally
    LConn.Free;
  end;
end;
...
begin
  ...
  try
    ...
    AssertTrue(Format('reenvio deveria entregar >= 100 msgs; entregou %d', [LCount]),
      LCount >= 100);
  finally
    LPubChan.Free;
    LPubConn.Free;
...
initialization
  RegisterTest(TAMQPReconnectIntegrationTests);
