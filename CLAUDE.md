...
3. ~~TLS multiplataforma via OpenSSL.~~ **Concluído** — `AMQP.Transport.OpenSSL.pas` (ver "Peças novas"), opt-in via `-dAMQP_OPENSSL`, validado no Linux x86_64 (container Debian, libssl 3): SmokeTest `--tls` PASS nos 7 passos (reconexão sobre TLS inclusa) e suíte FPCUnit de integração 24/24 com os 2 testes TLS conectando de verdade (inclusive a rejeição de cert self-signed com `TlsVerifyPeer=True`, que levanta `EAMQPTls` como no SChannel). No Windows, tudo revalidado: build padrão (SChannel) intacto — SmokeTest plain e `--tls` PASS, 80/80 + 24/24 — e o build com `-dAMQP_OPENSSL` compila.
...
5. Validação em Linux — **parcialmente concluída** (x86_64, container Debian/Docker, FPC 3.2.2, contra o RabbitMQ real do `docker/docker-compose.yml`, compartilhando o network namespace do container do broker pra `localhost` resolver certo): lib inteira compila limpo; `SmokeTest` PASS nos 7 passos (com reconexão); suítes FPCUnit 80/80 unitários e 24/24 integração (TLS incluso via OpenSSL com `-dAMQP_OPENSSL` — item 3 concluído; sem a diretiva os 2 de TLS se auto-ignoram e a suíte segue 24/24). Ver gotchas de `cthreads`, do runner FPCUnit console-only e de SIGPIPE acima. `Retaguarda` conectava via `TAMQPConnectionParams.LocalhostTls` por padrão (só funcionava no Windows) — trocado para `Localhost` puro, igual `AutorizadorSim`, pra dar pra testar o par no Linux também; ambos precisaram do mesmo `cthreads`. Testado o par completo (publica + consome concorrente) contra o broker real. Falta: ARM e os samples VCL/LCL (dependem de widgetset instalado).
